...
/** Called only once Slack, the Codex handshake, and the control socket are ready. */
export function recoverRestart(bridge: Bridge, invocation = process.env.INVOCATION_ID, now = Date.now()): void {
  ...
  bridge.store.queueRestartNotice(restart.id,
    ...
    + 'The Codex app-server handshake, Slack connection, and local scheduler control socket are ready. '
